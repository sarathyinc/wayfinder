...
- **Tasks** — Suggested sequences of actions, often used for onboarding.
...
1. **Auto-discovery** (default) — The `discover` step walks your routes, forms, and navigation configuration.
2. **Annotations** (optional, progressive enhancement) — Using `defineAction()` and `defineTask()` for higher fidelity and agentic behavior on important flows.
