...
- CI can run the drift gate to ensure the graph is up to date when code changes.
...
const provider = createProvider({ provider: 'mock' }); // or 'openai', etc.

export async function POST(req: NextRequest) {
  const body = await req.json();

  // IMPORTANT: Resolve persona from the authenticated session
  const personas = ['user']; // replace with real role resolution
...
Persona filtering is a **security boundary**. Never trust the client to tell you who they are.
