...
The assistant will respond using the capability graph generated for the demo (including suggested onboarding tasks).

## What Just Happened?

Behind the scenes, the following occurred:

1. A deterministic **discover** step walked the demo’s routes and components (including `deriveAvailable`).
2. A **compile** step (using a mock LLM provider) generated descriptions, steps, synonyms, and a global flow pass for suggested tasks (structure-hash cached).
3. Annotations (if an `assist-annotations.*` file exists) were merged last.
4. The resulting `capability_graph.json` (with `tasks`) was committed and is served at runtime.
5. The `<assist-widget>` (a Web Component) talks to a small API route powered by `createAssistHandler` + core `handleAssistChat` (server persona resolution, scale guard, kill-switch).
6. On first open the widget attempts to load persona tasks for a proactive checklist.
