...
import { loadGraph, computeGraphStructureHash, type CapabilityGraph } from "@wayfinder/core";
import { discoverCommand } from "./discover.js";

export async function compileCommand(dir = ".", options: { provider?: string } = {}) {
  const root = dir;
  const providerName = (options.provider || "mock") as "mock" | "openai" | "anthropic" | "ollama";
  ...
  for (const r of manifest.routes) {
    ...
    if (cached) {
      // use cache
      pages.push({ routeKey: r.routeKey, title: r.title || r.routeKey, personas: r.personas, available: true });
      actions.push({ id: `${r.routeKey}.view`, route: r.routeKey, label: r.title || "View", personas: r.personas, effect: "navigate", params: [], synonyms: [], steps: cached.steps || [], spotlight: [] });
      continue;
    }

    const compiled = await provider.compilePerRoute({ routeKey: r.routeKey, source: r.sourceBundle, filePaths: r.filePaths });
  ...
  const suggested = await provider.suggestTasks({
    actions: actions.map(a => ({ id: a.id, route: a.route, personas: a.personas })),
    transitions,
  ...
  const finalTasks = tasks.length > 0 ? tasks : [
    {
      ...
      sequence: actions.length > 1 ? [actions[0].id, actions[1]?.id || actions[0].id] : [],
  ...
  writeFileSync(join(root, "capability_graph.json"), JSON.stringify(graph, null, 2) + "\n");
  writeFileSync(cachePath, JSON.stringify(cache, null, 2) + "\n");

  console.log(`[assist] Wrote capability_graph.json with ${actions.length} actions and ${finalTasks.length} tasks.`);
}
